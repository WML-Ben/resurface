...
/**
 * Added beforeSend segment to ajax request (line: 239, 633, 752) to pass token
 * Added response = $.parseJSON(response); on those spots to success and error functions to handle response correctly
 * Removed response (is undefined) parameter on onAfterSelectImage (line 272)
 *
 * Jose Vidal - 2015-09-11
 */

/**
 * Added define row id and oldImageName to allow update row and delete previous image if it is not used as temp upload
 * (line 647)
 *
 * Jose Vidal - 2017-02-24
 */

/**
 * Added "column" (tabe field) to port data on removeImage (line 793)  on saveImage (line 662)
 *
 * Jose Vidal - 2018-05-25
 */
	...
	$.html5imageupload.prototype = {
		_init: function() {
			...
			if (!empty(options.image) && options.editstart == false) {
				...
				$(element).data('newFileName', options.image).append($('<img />').addClass('preview').attr('src',options.image));
		...
		handleFile: function(event, element) {
			...
			if (options.removeurl != null && !empty($(element).data('newFileName'))) {
				...
				$.ajax({
					...
					{
						request.setRequestHeader('X-XSRF-TOKEN', options.token);
					},
					url: options.removeurl,
					data: {image: $(element).data('newFileName') },
...
// saveImage:
			...
			$.ajax({
				...
				url: options.url,
				data: $.extend(obj, {
                    'oldImageName': $(element).data('oldImageName'),
				...
				success: function(response) {
					...
					if (response.status == "success") {
						...
						$(element).find('.tools .saving').remove();
						$(element).find('.tools').children().toggle();
						$(element).data('originalFileName', response.originalFileName);
						$(element).data('newFileName', response.newFileName);
						if (options.canvas != true) {
...
// when remove:
				...
				$.ajax({
					...
					url: options.removeurl,
					data: {
						image       : $(element).data('newFileName'),
						imageName   : $(element).data('imagename'),
                        oldImageName: $(element).data('old-image-name'),
						id          : $(element).data('id'),
                        column      : $(element).data('column'),
