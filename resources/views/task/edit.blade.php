...
@section('breadcrumbs')
    <div class="mb15">
        <ul class="page-breadcrumb breadcrumb">
            ...
            <li><a href="{{ route('task_list') }}">Tasks</a><i class="fa fa-angle-right"></i></li>
        ...
        <div class="row">
            <div class="col-md-9 center-block">
                ...
                <div class="admin-form theme-primary">
                    <div class="panel">
                        {!! Form::model($task, ['url' => route('task_update', ['id' => $task->id]), 'method' => 'PATCH', 'id' => 'updateForm']) !!}
                            {!! Form::hidden('id', $task->id) !!}
                            @include('task._form', ['formTitle' => 'Update Task', 'submitButtonText' => 'Update'])
                        {!! Form::close() !!}
                    </div>
                </div>
            </div>
...
@section('js-files')
    <script>
        $(function(){
            ...
            $('#task_id').change(function(ev){
                $.ajax({
                    type:"post",
                    url: "{{ route('ajax_task_managers_fetch') }}",
                    data: {
                        task_id: $(this).val()
                    },
                    beforeSend: function (request){
            ...
            $('#cancel-button').click(function(ev){
                ev.preventDefault();
                window.location = "{{ route('task_list') }}";
            });
        });
    </script>
@stop