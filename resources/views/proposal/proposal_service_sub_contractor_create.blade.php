                                        ...
                                        <div class="portlet-body">
                                            ...
                                            <div class="row" id="required_fields_section">
                                                <div class="col-md-4 col-sm-4 admin-form-item-widget">
                                                    {{ Form::jSelect('vendor_id', $subContractorsCB, ['label' => 'Sub Contactor', 'selected' => ($service->subContractor->id ?? null), 'required' => true, 'class' => 'gui-input', 'iconClass' => 'fa fa-user', 'attributes' => ['id' => 'vendor_id']]) }}
                                                </div>
                                                <div class="col-md-2 col-sm-2 admin-form-item-widget">
                                                    {{ Form::jText('profit', ['label' => 'Profit', 'id' => 'profit', 'required' => true, 'iconClass' => 'fa fa-dollar']) }}
                                                </div>
                                                <div class="col-md-2 col-sm-2 admin-form-item-widget">
                                                    {{ Form::jText('cost_per_day', ['label' => 'Cost', 'id' => 'cost_per_day', 'required' => true, 'iconClass' => 'fa fa-dollar']) }}
                                                </div>
                                                <div class="col-md-2 col-sm-2 admin-form-item-widget bk-light-blue">
                                                    {{ Form::jText('additive', ['label' => 'Over Head %', 'id' => 'additive', 'required' => true, 'iconClass' => 'fa fa-percent']) }}
                                                </div>
                                                <div class="col-md-2 col-md-2 col-sm-4 admin-form-item-widget">
                                                    {{ Form::jShow($service->subContractor->overhead ?? '&nbsp;', ['label' => 'Sub Over Head %', 'id' => 'show_sub_over_head', 'class' => 'icon-class-container', 'iconClass' => 'fa fa-percent']) }}
                                                </div>
                                            </div>
                                            <div class="row pb20">
                                            ...
                                            <div class="row pb20">
                                                <div class="col-md-12 admin-form-item-widget">
                                                    {{ Form::jTextarea('description', ['label' => 'Job Description', 'id' => 'description', 'required' => true, 'iconClass' => 'fa fa-bookmark']) }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="row">
                                                ...
                                                <div id="total_overhead_container" class="col-md-3 col-sm-3 admin-form-item-widget">
                                                    {{ Form::jShow('&nbsp;', ['label' => 'Over Head', 'hint' => '', 'id' => 'show_overhead', 'iconClass' => 'fa fa-dollar', 'class' => 'icon-class-container']) }}
                                                </div>
                                            </div>
...
@section('js-files')
    <script>
        $(function(){
            ...
            $('#vendor_id').change(function(ev){
                $.ajax({
                    type:"post",
                    url: "{{ route('ajax_sub_contractor_fetch') }}",
                    data: {
                        sub_contractor_id: $(this).val()
                    },
                    beforeSend: function (request){
                        PNotify.removeAll();
                        showSpinner();
                    ...
                    success: function(response){
                        if (response.success) {
                            $('#show_sub_over_head').html(response.sub_contractor.overhead);
                            if (!$('#additive').val() || $('#additive').val() == '') {
                                $('#additive').val(response.sub_contractor.overhead);
                            }
                            $('#total_overhead_container').find('.hint').html('(calculated at '+ $('#additive').val() +'%)');
                        } else {
                            pnAlert({
                                type: 'error',
                                title: 'Error',
        ...
        function calculateAll()
        {
            ...
            var costPerDay = normalizeFloat($('#cost_per_day').val());
            var overHead = normalizeFloat($('#additive').val());

            var materialsTotalCost = costPerDay;
            ...
            var soh = 1 - (overHead / 100);
            var overhead = (otCost / soh) - otCost;
