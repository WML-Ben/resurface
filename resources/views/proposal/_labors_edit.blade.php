                ...
                <table class="table table-bordered list-table sortable-table">
                    ...
                    <tbody class="multi-section">
                        @foreach ($obj as $labor)
                            @php
                                $existingLabor = $orderService->labors()->where('labor_id', $labor->id)->first();
                            @endphp
                            <tr data-id="{{ $labor->id }}" data-rate="{{ $labor->rate }}">
                                <td class="td-checkbox">
                                    <div class="radio-checkbox-container vertical">
                                        <label for="labor_id_{{ $labor->id }}" class="radio-checkbox-item">
                                            <input id="labor_id_{{ $labor->id }}" class="checkbox" type="checkbox" value="{{ $labor->id }}"{{ !empty($existingLabor->id) ? ' checked' : '' }}>
                                            <i class="fa fa-square-o"></i>
                                            <i class="fa fa-check-square-o"></i>
                                        </label>
                                    </div>
                                ...
                                <td class="td-number">
                                    <label for="labor_quantity_{{ $labor->id }}" class="field">
                                        <input class="gui-input quantity tc" id="labor_quantity_{{ $labor->id }}" name="labor_quantity[]" type="text" value="{{ $existingLabor->quantity ?? '' }}">
                                    </label>
                                </td>
                                <td class="td-number">
                                    <label for="labor_days_needed_{{ $labor->id }}" class="field">
                                        <input class="gui-input days-needed tc" id="labor_days_needed_{{ $labor->id }}" name="labor_days_needed[]" type="text" value="{{ $existingLabor->days_needed ?? '' }}">
                                    </label>
                                </td>
                                <td class="td-number">
                                    <label for="labor_hours_per_day_{{ $labor->id }}" class="field">
                                        <input class="gui-input hours-per-day tc" id="labor_hours_per_day_{{ $labor->id }}" name="labor_hours_per_day[]" type="text" value="{{ $existingLabor->hours_per_day ?? '' }}">
                                    </label>
                                </td>
                            </tr>
                        @endforeach
