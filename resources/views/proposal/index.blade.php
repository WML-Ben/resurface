                    ...
                    <div class="portlet-body">
                        <div class="clearfix">
                            <div class="col-sm-12 col-md-7 pl0 ml0 pr0 mr0 mb15">
                                    ...
                                    <span class="select-container">
                                        ...
                                        <select id="status_filter">
                                            @foreach ($statusCB as $key => $value)
                                                <option value="{{ $key }}"{!! !empty($statusId) && $statusId == $key ? ' selected="selected"' : ''!!} data-url="{{ route('proposal_list', array_merge(Request::query(), ['statusId' => $key])) }}">{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    </span>
                                @endif
                                ...
                                {{--
                                <span class="select-container">
                                    <label for="start_filter" class="select-label">Start</label>:
                                    <select id="start_filter">
                                        @foreach ($startsCB as $key => $value)
                                            <option value="{{ $key }}"{!! !empty($startId) && $startId == $key ? ' selected="selected"' : ''!!} data-url="{{ route('proposal_list', array_merge(Request::query(), ['startId' => $key])) }}">{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </span>
                                --}}
                            ...
                            <table class="table table-bordered list-table scrollable">
                                <thead>
                                    <tr>
                                        ...
                                        @endif
                                        {{--
                                        <th class="td-sortable tc td-date">{!! SortableTrait::link('orders.start_id|order_starts.name', 'Lead Status') !!}</th>
                                        ...
                                        <th class="td-sortable tc td-date">{!! SortableTrait::link('created_at', 'Created At') !!}</th>
                                        --}}
                                ...
                                @foreach ($proposals as $proposal)
                                    <tr data-id="{{ $proposal->id }}" class="not-tag {{ !empty($proposal->alert) ? ' alert' : (!empty($proposal->disabled) ? ' disabled' : (!empty($proposal->status_id) ? ' status-'. $proposal->status_id : '')) }}"{!! !empty($proposal->aged_period->background_color) ? ' style="background-color:'. $proposal->aged_period->background_color .'"' : '' !!}>
                                        ...
                                        @if (!empty($statusCB))
                                            <td class="tc td-date">{{ $proposal->status->name ?? 'unknown' }}</td>
                                        @endif
                                        <td class="centered actions all-time-visible-actions">
                                            ...
                                            @endif
                                            {{--
                                            @if (auth()->user()->hasPrivilege('add-note-proposal'))
                                                <a href="javascript:void(0);" class="not-yet-available action" data-action="route" data-route="{{ route('proposal_note_create', ['id' => $proposal->id]) }}">Add Notes</a>
                                            @endif
                                            @if (auth()->user()->hasPrivilege('add-upload-proposal'))
                                                <a href="javascript:void(0);" class="not-yet-available action" data-action="route" data-route="{{ route('proposal_upload_form', ['id' => $proposal->id]) }}">Add Upload</a>
                                            @endif
                                            @if (auth()->user()->hasPrivilege('change-status-proposal'))
                                                <a href="javascript:void(0);" class="not-yet-available action" data-action="route" data-route="{{ route('proposal_status_edit', ['id' => $proposal->id]) }}">Change Status</a>
                                            @endif
                                            @if (auth()->user()->hasPrivilege('show-proposal'))
                                                <a href="javascript:void(0);" class="not-yet-available action" data-action="route" data-route="{{ route('proposal_history', ['id' => $proposal->id]) }}">View History</a>
                                            @endif
                                            @if (auth()->user()->hasPrivilege('print-proposal'))
                                                <a href="javascript:void(0);" class="not-yet-available action" data-action="route" data-route="{{ route('proposal_print', ['id' => $proposal->id]) }}">Print </a>
                                            @endif
                                            @if (auth()->user()->hasPrivileges(['disable-proposal', 'delete-proposal']))
                                                @if (auth()->user()->hasPrivilege('disable-proposal'))
                                                    <a href="javascript:void(0);" class="action" data-action="route" data-route="{{ route('proposal_toggle_status', ['id' => $proposal->id]) }}">{{  empty($proposal->disabled)? 'Disable' : 'Enable'}}</a>
                                                @endif
                                                @if (auth()->user()->hasPrivilege('delete-proposal'))
                                                    <a href="javascript:void(0);" class="action" data-action="delete" data-id="{{ $proposal->id }}">Delete</a>
                                                @endif
                                            @endif
                                             --}}
                                        </td>
